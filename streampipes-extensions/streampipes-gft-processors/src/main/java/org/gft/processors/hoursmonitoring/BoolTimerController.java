...
public class BoolTimerController extends StandaloneEventProcessingDeclarer<BoolTimerParameters> {
  ...
  public DataProcessorDescription declareModel() {
    return ProcessingElementBuilder.create("org.gft.processors.hoursmonitoring")
            ...
            .outputStrategy(OutputStrategies.append(
                    EpProperties.numberEp(Labels.withId(MEASURED_TIME_ID), "measured_time", SO.Number),
                    EpProperties.numberEp(Labels.withId(MONTHLY), "hours_monthly", SO.Number),
                    EpProperties.numberEp(Labels.withId(DAILY), "hours_daily", SO.Number),
                    EpProperties.numberEp(Labels.withId(WEEKLY), "hours_weekly", SO.Number)
            ))
            .build();
  }

  @Override
  public ConfiguredEventProcessor<BoolTimerParameters> onInvocation(DataProcessorInvocation graph, ProcessingElementParameterExtractor extractor) {
    ...
    double outputDivisor= 60000.0;

    if (outputUnit.equals(HOURS)) {
      outputDivisor = 60000.0*60;
    }

    BoolTimerParameters params = new BoolTimerParameters(graph, invertFieldName, measureTrue, outputDivisor);

    return new ConfiguredEventProcessor<>(params, BoolTimer::new);
  }
}
